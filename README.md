...
This example Extension demonstrates the functioning of Extension Module extensions within this CMS. Its explaining the use of various folders and provides a skeleton template for creating your own Theme modules.

You may download the example module here: [Download](https://raw.githubusercontent.com/bugfishtm/suitefish-cms/refs/heads/main/_packages/_extension-1.10.100.zip)

For possibilities on triggers, injections and more take a look at trigger and view example files inside the official administrator module.

## 🛠️ Installation

1. Method: Login to your suitefish instance and browse our official store on your page. If you find the module you are looking for just download the package. Then navigate to the Extension Package Area and enable the downloaded Extension.

2. Method: Navigate to the Extension Package Area inside the suitefish instance. Go to the upload section and upload the modules .zip file.  Then navigate to the Extension Package Area and enable the downloaded Extension.
...
| `./_assets/` | Folder for Extensions Assets Files. | Optional |
...
| `./_wfc/` | Folder for site builder blocks to extend the system. | Optional |
| `./_wfc/_examples` | Examples for WFC Functionalities | Optional |  
...
- Public image modules should have an Rname starting with **"ext"**.
...
You can access this array keys at: `$object["extension"]` for example. 
...
| `$x["type"]`             | Module type identifier (1=Site, 2=Extension, 3=Image, 4=Windows, 5=Docker, 6=Theme).                | `2` (for extension modules) |
...
| `$x["docker_valid"]`            | True if this extension can be used inside a docker container. | False if not. |
